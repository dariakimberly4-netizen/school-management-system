...
function openModule(name){current=name;$("home").classList.add("hidden");$("workspace").classList.remove("hidden");$("wsTitle").textContent=name;$("search").value="";const isStudent=$("role").value==="Student";if(isStudent&&studentSubmenus[name]){ $("addBtn").style.display="none";$("search").style.display="none";$("content").innerHTML='<div class="studentSubmenu">'+studentSubmenus[name].map(x=>'<button type="button" class="subMenuBtn"><b>'+x+'</b><span>Open →</span></button>').join("")+'</div>';document.querySelectorAll(".subMenuBtn").forEach(b=>b.onclick=()=>{$("content").innerHTML='<div class="studentDetail"><button type="button" class="subBack">← Back to '+name+'</button><h2>'+b.querySelector("b").textContent+'</h2><p>This student workspace is ready for school data. Official grades, attendance, payments and profile records are view-only.</p></div>';$("content").querySelector(".subBack").onclick=()=>openModule(name)});return}$("addBtn").style.display="";$("search").style.display="";data[current]??=[];renderRows()}
